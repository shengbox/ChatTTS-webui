<template>
  <a-layout class="layout">
        ...
        <div style="display: flex; flex-direction: column; margin-left: 10px">
          <div style="display: flex; margin-bottom: 10px">
            ...
            <a-radio-group v-model:value="voice" style="display: flex; margin-left: 10px" >
              <a-radio-button value="2222">{{ language === "en" ? "Male Voice" : "男声" }}</a-radio-button>
              <a-radio-button value="1942">{{ language === "en" ? "Female Voice" : "女声" }}</a-radio-button>
              <a-radio-button value="0">{{ language === "en" ? "Random Voice" : "随机" }}</a-radio-button>
            </a-radio-group>
          </div>
    ...
    <a-layout-footer style="text-align: center">
      Developed based on
      <a href="https://github.com/2noise/ChatTTS/">ChatTTS</a> by
      <a href="https://2noise.com/">@2noise</a>, web developed by
      <a href="https://github.com/Gouryella">@Gouryella</a>
    </a-layout-footer>
  </a-layout>
</template>
...
const generateRandomNumber = () => {
  return Math.floor(Math.random() * (7869 - 1000 + 1)) + 1000;
}

const generate = async () => {
  loading.value = true;
  audioUrl.value = false;
  let seed = parseInt(voice.value)
  if (seed==0) {
